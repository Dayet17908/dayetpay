...
<%@page import="com.sevenpay.bms.basemanager.merchant.auding.AgencyCtroller.WechatAudingPath" %>
...
<%@page import="com.sevenpay.bms.basemanager.merchant.auding.bean.AgencyAudingPath"%>
	...
	if(type =='edit'){
		...
		$("#cashier0").empty();
		   var custId=$(obj).parent().find('#custId_01').val();
			...
			$("#updateMerchantModel #businessPhotoImageDiv1").attr('src',
	 				"<%=request.getContextPath() + AgencyPath.BASE 
				+ AgencyPath.PREVIEWAGENCYIMAGE %>?custId=" + custId + "&certifyType=02");
			...
			$.post(window.Constants.ContextPath +'<%=WechatAudingPath.BASE + WechatAudingPath.WECHATEDIT%>',{
	  			...
	  			if(data.custType =='0'){
 	 				...
 	 				$("#updateMerchantModel #hideBusinessTerm").hide();
 	 			   }
 	 			 if(data.custType=='1'){
 	 				$("#hiddenbusinessPhotoDiv").show();
 	 				$("#updateMerchantModel #hideBusinessTerm").show();
 	 			  }
