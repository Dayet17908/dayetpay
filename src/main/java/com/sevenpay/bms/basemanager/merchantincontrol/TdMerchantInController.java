package com.sevenpay.bms.basemanager.merchantincontrol;
...
import com.sevenpay.bms.basemanager.merchantincontrol.service.TdMerchantInControlService;
...
public class TdMerchantInController {
	...
	public ModelAndView list(MerchantInControl queryBean){
		logger.info("====商户网关维护查询对象 ：{}",JSONObject.toJSONString(queryBean,true));
		ModelAndView mv = new ModelAndView(TdMerchantInControPath.BASE + TdMerchantInControPath.LIST);		
		mv.addObject("merchantInControlList", JSONObject.toJSON(service.selectMerchantInControlList(queryBean)));
