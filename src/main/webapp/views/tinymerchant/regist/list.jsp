						  	...
						  	<td class="td-left">商户登陆状态：</td>
								<td class="td-right">
									<select id="merchantState" name ="merchantState" class="width-28">
										...
										<option value="06">商户协议待录入 </option>
									</select>								
								</td>
						  </tr>
						 <tr class="businessLicensePhotoTr">
...
function confirmMerchantInfo(){
	...
	var flag;
	// 校验商户名称
	flag = Register.validateCustName($("#custName_tc").val(),$("#custNameLabel"));
	...
	var custName = $("#custName_tc").val().trim(); 							// 客户名称
...
/* 	$("#updateMerchant #businessPhotoImageDiv").attr("src","");
	...
	if(option == 'edit'){
		...
		$('#updateMerchant input,select').prop("disabled", false);categoryTypeId
	...
	if(merchant.businessLicense != '' && merchant.businessLicense != null){
		$("#updateMerchant #businessPhotoImageDiv").attr("src",//
				"<%=request.getContextPath() + TinyMerchantPath.BASE 
				+ TinyMerchantPath.PREVIEWTINYMERCHANTIMAGE %>?custId=" + custId + "&certifyType=02");
	}
	
	$("#updateMerchant #certAttribute1ImageDiv").attr("src",//
			"<%=request.getContextPath() + TinyMerchantPath.BASE 
			+ TinyMerchantPath.PREVIEWTINYMERCHANTIMAGE %>?custId=" + custId + "&certifyType=04&front=0");
	$("#updateMerchant #certAttribute2ImageDiv").attr("src",//
			"<%=request.getContextPath() + TinyMerchantPath.BASE 
